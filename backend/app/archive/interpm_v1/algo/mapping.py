            ...
            if pro in group[ocel.object_type_column].values:
                ...
                for i in range(len(group)):
                    if group[ocel.object_type_column].iloc[i] in object_types:
                        process_object_type_mapping[pro].add(group[ocel.object_type_column].iloc[i])

    return process_object_type_mapping

def map_process_to_event_activity(ocel: OCEL) -> Dict[str, set[str]]:
    object_types = get_object_types(ocel)
    processes = get_processes(ocel)
    objects = get_objects(ocel)
    process_event_activity_mapping = dict()
    ...
    return process_event_activity_mapping